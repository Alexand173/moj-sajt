            ...
            {songs[0] && (
              <article className="group overflow-hidden border border-accent-red bg-ink shadow-[0_0_40px_-8px_rgb(230_57_70_/_0.45)]">
                <div className="aspect-video w-full bg-ink">
                  {getVideoSrc(songs[0].youtube_id) ? <iframe src={getVideoSrc(songs[0].youtube_id) || ''} title={`${songs[0].title} preview`} loading="lazy" className="h-full w-full" allow="autoplay; encrypted-media" allowFullScreen /> : <div className="flex h-full items-center justify-center text-xs font-bold tracking-[0.16em] text-white/45 uppercase">Video unavailable</div>}
                </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
              {songs.slice(1, 3).map((song, index) => (
                <article key={song.id} className="group overflow-hidden border border-line bg-ink transition-colors hover:border-accent-red">
                  <div className="aspect-video bg-ink">{getVideoSrc(song.youtube_id) ? <iframe src={getVideoSrc(song.youtube_id) || ''} title={`${song.title} preview`} loading="lazy" className="h-full w-full" allowFullScreen /> : <div className="flex h-full items-center justify-center text-xs text-white/45">Video unavailable</div>}</div>
