...
export default function LeaderCard({ entry }: LeaderCardProps) {
  ...
  return (
    <article className="group relative flex min-w-0 flex-col gap-5 overflow-hidden border border-awards-gold bg-ink-elevated p-5 shadow-[0_0_50px_-12px_rgb(255_193_7_/_0.45)] transition-colors sm:flex-row sm:items-center sm:gap-6 sm:p-6">
      ...
      <div className="size-20 shrink-0 overflow-hidden bg-ink sm:size-24">{entry.image ? <img src={entry.image} alt={entry.title} loading="lazy" decoding="async" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" /> : <span className="flex h-full w-full items-center justify-center text-2xl font-black text-white/25">{entry.artist_name.charAt(0)}</span>}</div>
